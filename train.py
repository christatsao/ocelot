...
def tiss_training_loop(args,
        ...
        if model.outputChannel == 1:
            ...
            criterion = DiceCELoss(softmax=True, to_onehot_y=True).to(device)
        ...
        for epoch in range(args.epochs):
            ...
            with tqdm(total=len(train), desc=f'Epoch {epoch+1}/{args.epochs}') as progress_bar:
                ...
                train_loss = (epoch_loss/N_batches_train)/model.outputChannel
...
def main(args):
    ...
    #First we need to specify some info on our model
    if args.modelType == 'ResNetUnet':
        model = ResNetUnet(args).to(my_device)
    elif args.modelType == 'Unet':
        model = Unet(args, bilinear=False).to(my_device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-pretrained'       ,type=bool,  action="store", dest='pretrained'        , default=True        )
    parser.add_argument('-freezebackbone'   ,type=bool,  action="store", dest='freezebackbone'    , default=True        )
    ...
    parser.add_argument('-nepoch'           ,type=int  , action="store", dest='epochs'            , default=1000          )
    ...
    parser.add_argument('-modelType'        ,type=str  , action="store", dest='modelType'        , default='ResNetUnet'      )
